...
constexpr int mode_id=211,model_id=212,provider_id=213,record_id=214,sample_id=215;
...
public:
    ...
    HWND mode{},load_model{},provider{},record{},sample{};
    HFONT font{};
    UINT dpi{96};
    ...
    void create(HWND hwnd){
        ...
        heading=control(L"STATIC",L"SekiroVisionAI  |  Temporal AI + Auto Dodge\r\nF8: toggle in game   |   F9: disable + release   |   F10: manual input test (when enabled)",0,0);
        ...
        mode=control(L"COMBOBOX",L"",CBS_DROPDOWNLIST|WS_TABSTOP,mode_id);
        load_model=control(L"BUTTON",L"Load ONNX model...",WS_TABSTOP,model_id);
        provider=control(L"COMBOBOX",L"",CBS_DROPDOWNLIST|WS_TABSTOP,provider_id);
        record=control(L"BUTTON",L"Record Training Samples (F7)",BS_AUTOCHECKBOX|WS_TABSTOP,record_id);
        sample=control(L"BUTTON",L"Save sample (F6)",WS_TABSTOP,sample_id);
        ...
        for(auto h:{heading,windows,refresh,start,stop,disable,sensitivity,direction,roi_reset,openlog,folder,metrics,events,mode,load_model,provider,record,sample})
        ...
        for(auto label:{L"CV heuristic fallback",L"Temporal ONNX model"})SendMessageW(mode,CB_ADDSTRING,0,reinterpret_cast<LPARAM>(label));
        for(auto label:{L"CPU",L"DirectML GPU",L"CUDA (GPU build only)"})SendMessageW(provider,CB_ADDSTRING,0,reinterpret_cast<LPARAM>(label));
        ...
        SendMessageW(mode,CB_SETCURSEL,config.detector_mode,0);SendMessageW(provider,CB_SETCURSEL,config.provider=="CUDA"?2:config.provider=="DirectML"?1:0,0);
    ...
    void apply_font(){
        ...
        for(auto h:{heading,windows,refresh,start,stop,disable,sensitivity,direction,roi_reset,openlog,folder,metrics,events,mode,load_model,provider,record,sample})SendMessageW(h,WM_SETFONT,reinterpret_cast<WPARAM>(next),TRUE);
    ...
    void layout(){
        ...
        move(mode,18,150,205,180);move(load_model,236,149,178,32);move(provider,428,150,208,180);
        move(record,18,190,270,27);move(sample,304,186,160,30);
        const double aspect=latest.preview.color?static_cast<double>(latest.preview.color->height)/latest.preview.color->width:9.0/16;
        const int image_width=std::min({640,(width-54)*57/100,static_cast<int>(360/aspect)}),image_height=static_cast<int>(image_width*aspect);
        preview={px(18),px(246),px(18+image_width),px(246+image_height)};
        move(metrics,std::max(550,image_width+36),192,width-std::max(550,image_width+36)-18,452);
        const int log_top=std::max(695,246+image_height+95);
    ...
    void update(){
        auto cap=capture.snapshot();const auto old_color=latest.preview.color;latest=dodge.snapshot();
        if(latest.preview.color&&(!old_color||old_color->width!=latest.preview.color->width||old_color->height!=latest.preview.color->height))layout();
        ...
        EnableWindow(sensitivity,config.detector_mode==0);
        ...
        text<<(latest.input.enabled?L"AUTO DODGE: ON":L"AUTO DODGE: OFF")<<L"  |  "<<wide(latest.input.reason)
            ...
            <<L"\r\n"<<cap.adapter<<L"\r\n"<<cap.detail
            <<L"\r\nSource: "<<latest.preview.source_width<<L" x "<<latest.preview.source_height
            <<L"  color: "<<(latest.preview.color?std::to_wstring(latest.preview.color->width)+L" x "+std::to_wstring(latest.preview.color->height):L"waiting")
            <<L"\r\n\r\nThreat: "<<wide(latest.threat.state)<<L"  |  "<<wide(latest.threat.reason);
        if(config.detector_mode) {
            text<<L"\r\nModel: "<<wide(latest.model.version.empty()?"no trained model installed":latest.model.version)
            ...
            if(latest.prediction.valid) {
                ...
                if(latest.prediction.tti_supported)text<<L"\r\nTTI at frame: "<<fixed(latest.prediction.tti_ms,0)<<L" +/- "<<fixed(latest.prediction.tti_uncertainty_ms,0)
                    <<L" ms; now: "<<fixed(latest.prediction.tti_ms-(qpc_ms()-latest.prediction.source_ms),0)<<L" ms";
                else text<<L"\r\nTTI: unknown / unsupported by training labels";
                text<<L"\r\nInference: "<<fixed(latest.prediction.inference_ms)<<L" ms (host wall time)";
            } else text<<L"\r\nAttack p / threat p / TTI: unavailable  |  "<<wide(latest.prediction.reason);
            text<<L"\r\nTTI lead window: "<<config.temporal.lead_min_ms<<L" - "<<config.temporal.lead_max_ms<<L" ms (initial setting)";
        } else text<<L"\r\nCV FALLBACK: heuristic score "<<fixed(latest.motion.score)<<L"; quality "<<fixed(latest.motion.confidence)
            <<L"\r\nCamera dx/dy: "<<fixed(latest.motion.camera_dx,0)<<L" / "<<fixed(latest.motion.camera_dy,0)
            <<L"\r\n"<<wide(latest.motion.reason)<<L"  |  not a trained probability; TTI unknown";
        text<<L"\r\nPipeline: "<<fixed(latest.cpu_ms)<<L" ms  source age now: "<<(latest.has_frame?fixed(qpc_ms()-latest.preview.source_ms):L"unavailable")<<L" ms"
            ...
            <<L"\r\nRecorder: "<<wide(latest.recording.reason)<<L"  saved: "<<latest.recording.saved<<L" dropped: "<<latest.recording.dropped
            <<L"\r\nWatchdog: "<<(latest.input.watchdog_ready?L"ready":L"unavailable")<<L"  Logs: "<<(dodge.log()->healthy()?L"writing":L"write failed")
            <<L"\r\nBoss / distance / safe direction: unknown. Direction uses your selected keys.";
    ...
    void paint(){
        ...
        if(latest.preview.color){
            const auto& color=*latest.preview.color;
            BITMAPINFO info{};info.bmiHeader.biSize=sizeof(BITMAPINFOHEADER);info.bmiHeader.biWidth=color.width;info.bmiHeader.biHeight=-color.height;
        ...
        DrawTextW(dc,L"Lock on; drag enemy ROI; return to Sekiro and press F8.\r\nF7 record | F6 sample | F5 false positive | F4 missed attack (review labels).",-1,&note,DT_LEFT|DT_WORDBREAK);
    ...
    void command(int id,int code){
        if(closing)return;
        if(code==CBN_SELCHANGE&&(id==sensitivity_id||id==direction_id||id==mode_id||id==provider_id)){
            auto config=dodge.config();
            if(id==direction_id)config.direction=static_cast<int>(SendMessageW(direction,CB_GETCURSEL,0,0));
            else if(id==mode_id)config.detector_mode=static_cast<int>(SendMessageW(mode,CB_GETCURSEL,0,0));
            else if(id==provider_id){const auto index=SendMessageW(provider,CB_GETCURSEL,0,0);config.provider=index==2?"CUDA":index==1?"DirectML":"CPU";}
            else{
        ...
        else if(id==record_id)dodge.recording(SendMessageW(record,BM_GETCHECK,0,0)==BST_CHECKED);
        else if(id==sample_id)dodge.mark_sample("MANUAL_SAMPLE");
        else if(id==model_id){
            dodge.disable();std::wstring path(32768,L'\0');OPENFILENAMEW dialog{};dialog.lStructSize=sizeof(dialog);dialog.hwndOwner=window;
            dialog.lpstrFilter=L"Sekiro temporal ONNX (*.onnx)\0*.onnx\0\0";dialog.lpstrFile=path.data();dialog.nMaxFile=static_cast<DWORD>(path.size());
            dialog.Flags=OFN_FILEMUSTEXIST|OFN_PATHMUSTEXIST|OFN_NOCHANGEDIR;
            if(GetOpenFileNameW(&dialog)){path.resize(wcslen(path.c_str()));auto config=dodge.config();config.model_path=path;config.detector_mode=1;
                SendMessageW(mode,CB_SETCURSEL,1,0);dodge.configure(config);}
        }
...
LRESULT CALLBACK procedure(HWND window,UINT message,WPARAM wparam,LPARAM lparam){
    ...
    try{if(app)switch(message){
        ...
        case WM_GETMINMAXINFO:{auto* info=reinterpret_cast<MINMAXINFO*>(lparam);info->ptMinTrackSize={app->px(1100),app->px(870)};return 0;}
...
int WINAPI wWinMain(HINSTANCE instance,HINSTANCE,PWSTR command,int show){
    try{
        ...
        HWND window=CreateWindowExW(0,cls.lpszClassName,L"SekiroVisionAI - Auto Dodge MVP",WS_OVERLAPPEDWINDOW|WS_CLIPCHILDREN,
            CW_USEDEFAULT,CW_USEDEFAULT,MulDiv(1220,GetDpiForSystem(),96),MulDiv(970,GetDpiForSystem(),96),nullptr,nullptr,instance,&app);
        if(!window)return 1;
