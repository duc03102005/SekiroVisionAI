...
## SekiroVisionAI scope

Use only if compiling ONNX Runtime itself is justified by an accepted runtime decision. Run the following commands inside a separately pinned ONNX Runtime source checkout, never the SekiroVisionAI root. References to AGENTS.md and sibling skills below belong to that upstream checkout; this project does not import them. Inspect any referenced build scripts and dependencies before execution. This imported Markdown includes no build scripts or binaries and has not built ONNX Runtime. Prefer supported prebuilt packages when they meet the requirement. Do not start a source build during Milestone 0. Keep startup/build costs separate from live batch-one benchmarks.
