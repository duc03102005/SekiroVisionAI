...
// These checks alone do not establish an allowed path; validate the selected
// resource and any required containment/permissions at the call site.
...
- MSIX package identity is not an AppContainer sandbox guarantee for full-trust desktop apps; inspect the actual trust model and capabilities.
...
- Choose visual visibility and accessibility-tree participation for the intended behavior and verify with a screen reader; do not infer one from the other without testing.
...
Use the SDK-provided analyzers where sufficient. If a separate analyzer package is needed, pin and review its version; do not introduce a wildcard package restore.
...
Respect configured analyzer severities for the changed area and assess suppressions with a concrete reason; avoid unrelated cleanup.
...
Use FlowDirection and supported XAML alignment values. Do not invent Start/End enum members. Verify mirrored layouts and directional Margin/Padding in the actual frontend.
